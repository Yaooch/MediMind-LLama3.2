...
def main():
    ...
    def return_prompt_and_responses(examples) -> Dict[str, str]:
        ...
        """
        prompts = []
        n_samples = len(examples["question"])
        # 2. 安全获取字段 (如果没 system，就填空字符串；如果没 history，就填空列表)
        systems = examples.get("system", [""] * n_samples)
        histories = examples.get("history", [[]] * n_samples)
        questions = examples["question"]
        for system, history, question in zip(systems, histories, questions):
