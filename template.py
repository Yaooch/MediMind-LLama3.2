...
class Conversation:
    ...
    #             convs.append(bot_resp)
    #     return convs
    def _format_example(self, messages, system_prompt=""):
        # ================= 1. 定义 Llama3 的标准格式 (硬编码兜底) =================
        # 防止 Conversation 类没有 system_format 属性导致报错
        default_system_fmt = "<|start_header_id|>system<|end_header_id|>\n\n{content}<|eot_id|>"
        default_user_fmt = "<|start_header_id|>user<|end_header_id|>\n\n{content}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n"
        default_assist_fmt = "{content}<|eot_id|>"
        
        # 尝试安全地获取属性，如果取不到(AttributeError)，就根据模板名称决定
        # 使用 getattr(self, '属性名', 默认值) 来避免报错
        sys_fmt = getattr(self, "system_format", None)
        usr_fmt = getattr(self, "user_format", None)
        ast_fmt = getattr(self, "assistant_format", None)

        # 如果属性不存在，且当前是 llama3，则强行使用标准格式
        if "llama3" in self.name.lower():
            if not sys_fmt: sys_fmt = default_system_fmt
            if not usr_fmt: usr_fmt = default_user_fmt
            if not ast_fmt: ast_fmt = default_assist_fmt
        # =======================================================================

        # ================= 2. 数据清洗 (兼容 Gradio 新版字典) =================
        cleaned_messages = []
        temp_user_msg = None

        for item in messages:
            if isinstance(item, (list, tuple)):
                cleaned_messages.append(list(item))
                ...
                if role == 'user':
                    ...
                    if temp_user_msg is not None:
                        cleaned_messages.append([temp_user_msg, text_content])
                        temp_user_msg = None
        
        # ================= 3. 格式化拼接 =================
        history = []
        # 3.1 处理 System Prompt
        if system_prompt:
            if sys_fmt:
                history.append(sys_fmt.format(content=system_prompt))
            else:
                history.append(system_prompt) # 只有内容，无格式

        # 3.2 处理对话轮次
        for turn_idx, [user_query, bot_resp] in enumerate(cleaned_messages):
            # 第一轮如果没加 System Prompt 且不是 llama3，可能需要在这里补（视旧逻辑而定，这里简化）
            
            # User
            if usr_fmt:
                # 这里的 stop_token 也要安全获取
                stop_wd = getattr(self, "stop_word", [])
                history.append(usr_fmt.format(content=user_query, stop_token=stop_wd))
            else:
            ...
            if bot_resp:
                if ast_fmt:
                    stop_wd = getattr(self, "stop_word", [])
                    history.append(ast_fmt.format(content=bot_resp, stop_token=stop_wd))
                else:
                    history.append(bot_resp)
